...
**i18n**: English is the source language — write `__('English text')`, never Spanish. `es`/`pt`
live in `scripts/translations/{es,pt}.json` keyed by that English string; `lang/{es,pt}.json` are
generated by `node scripts/generate-translations.mjs` and never edited by hand. The default
runtime locale is still `es` (`APP_LOCALE`), with `en` as the fallback so a missing key shows
English, not the raw key.
...
- **Legal**: `/privacidad` and `/terminos` (route names `legal.privacy` / `legal.terms`) →
  ...
  `lang/{es,en,pt}/legal.php` because these are paragraphs, not loose strings: keep the three
  files in step with each other, none is generated. Linked from the `nexo-footer` (so from every
  page, error pages included) and present in `sitemap.xml`.
